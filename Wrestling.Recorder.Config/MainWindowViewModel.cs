...
namespace Wrestling.Recorder.Config
{
        ...
        : BindableBase
    {
        ...
        public MainWindowViewModel(IVideoCaptureAccessor videoCaptureAccessor)
        {
            ...
            SaveConfigFileCommand = new DelegateCommand(SaveConfig);
        ...
        public DelegateCommand SaveConfigFileCommand { get; }
        ...
        private void SaveConfig()
        {
            ...
            {
                new DialogService().ShowMessageBox(this, $"Не выбрано устройство захвата видео!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }

            if (SelectedVideoFormat == null)
            {
                new DialogService().ShowMessageBox(this, $"Не выбран формат видео!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }

            if (SelectedOutputFormat == null)
            {
                new DialogService().ShowMessageBox(this, $"Не выбран формат выходного сигнала!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                ...
                VideoCaptureStartParams startParams = new VideoCaptureStartParams
                {
                    ...
                    VideoCaptureFormat = SelectedVideoFormat.Name,
                    VideoCaptureFormatUseBest = UseBestVideoInputFormat,
                    AudioCaptureDevice = SelectedAudioInputDevice != null ? SelectedAudioInputDevice.Name : null,
                    AudioCaptureDeviceFormat = SelectedAudioInputFormat,
                    AudioCaptureDeviceFormatUseBest = UseBestAudioInputFormat,
                    AudioOutputDevice = SelectedAudioOutputDevice,
                ...
                using (var writeFileStream = new StreamWriter(DEFAULT_CONFIG_FILENAME))
                {
                    var jsonWriter = new JsonTextWriter(writeFileStream);
                    var ser = new VisioForge.Shared.Newtonsoft.Json.JsonSerializer();
                    ser.Serialize(jsonWriter, startParams);
                    jsonWriter.Flush(); 
                }            
