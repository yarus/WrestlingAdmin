...
namespace EmguFFmpeg
{
    public unsafe class PixelConverter : FrameConverter<VideoFrame>
    {
        ...
        public PixelConverter(AVPixelFormat dstFormat, int dstWidth, int dstHeight, int flag = ffmpeg.SWS_BICUBIC)
        {
            DstWidth = dstWidth;
            DstHeight = dstHeight;
            DstFormat = dstFormat;
        ...
        public PixelConverter(MediaCodec dstCodec, int flag = ffmpeg.SWS_BICUBIC)
        {
        ...
        public PixelConverter(VideoFrame dstFrame, int flag = ffmpeg.SWS_BICUBIC)
        {
        ...
        public VideoFrame ConvertFrame(MediaFrame srcFrame)
        {
            ...
            if (pSwsContext == null && !isDisposing)
            {
                ...
                var convertedFrameBufferSize = ffmpeg.av_image_get_buffer_size(DstFormat, DstWidth, DstHeight, 1);
                var convertedFrameBufferPtr = Marshal.AllocHGlobal(convertedFrameBufferSize);
                var dstData = new byte_ptrArray4();
                var dstLinesize = new int_array4();

                ffmpeg.av_image_fill_arrays(ref dstData, ref dstLinesize, (byte*)convertedFrameBufferPtr, DstFormat, DstWidth, DstHeight, 1);
            }
