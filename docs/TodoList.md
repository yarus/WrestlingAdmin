...
- Матчи внутри группы — по `BracketFullNumber` (стабильная пара `RoundNumber.BracketNumber`), `TournamentImporter.cs:295`. Комментарий в коде явно объясняет почему не `MatchNumber`: «MatchNumber per-carpet и renumber'ится при перемещении группы — using it as the merge key would silently match wrong matches».
...
**Решение:** Закрыто через `ApplyGroupFieldChanges` (`TournamentImporter.cs:443`). Секретарь меняет `CarpetID` группы у себя → бампится `FieldsVersion`. На пирах:
- `local.CarpetID = remote.CarpetID` (line 454).
- `oldCarpet.Groups.Remove(local)` + `newCarpet.Groups.Add(local)` (lines 472-487) переподключают группу к нужному ковру в живом графе.
