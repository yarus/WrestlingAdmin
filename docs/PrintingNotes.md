...
Решение — передавать **фиксированную короткую строку** того же стиля, что в `PrintView.xaml.cs` («Печать»). Не интерполируй имя ковра/группы/чего угодно прямо в `documentName`. Контекст (какой ковёр, какая группа) живёт в самом теле PDF через биндинги (`Stat.MatLabel`, `SelectedGroup.Name` и т.п.) — этого достаточно.
