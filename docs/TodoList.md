...
### #8 — Импорт жёстко завязан на номер схватки (частично снято)
**Симптом:** если на одном ковре поменялась сетка → поехали номера → импорт между коврами начинает давать сбои.

**Текущее состояние (после #11 и #14):**
- Группы матчатся в `TournamentImporter.Apply` по `group.ID` (стабильный GUID) — `TournamentImporter.cs:223`.
- Матчи внутри группы матчатся по `MatchNumber` — `TournamentImporter.cs:237`. Это всё ещё нестабильный ключ при перепосеве.
- В реальных сценариях симптом снят: после фикса #11 нумерация на ковре стабилизирована (двойных номеров нет), а версионная модель #14 разруливает гонки независимо от ключа сопоставления. Пока ковры не делают перепосев в живом турнире — текущий ключ работает корректно.

**Что осталось:** перейти на стабильный match-Id (GUID на `WrestlingMatch`) в качестве ключа сопоставления. Это станет обязательным, когда появится админ-операция «опубликовать пересобранную сетку» (#3) — там номера у пиров поедут и MatchNumber-ключ начнёт ломать импорт.

**Сложность:** M. Зависимость: имеет смысл делать вместе с #3 (всё равно туда же придёт миграция структуры).
